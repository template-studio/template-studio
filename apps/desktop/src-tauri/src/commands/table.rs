...
pub async fn cmd_import_tables_from_datasource(
    ...
    import_tables_from_datasource(db.pool(), project_id, datasource_id, &database_name).await
}
...
pub async fn db_reorder_columns(
    ...
    for (index, column_id) in column_ids.iter().enumerate() {
        db.update_column_position(*column_id, (index + 1) as i32)
            .await
...
pub async fn cmd_parse_sql_and_create(
    ...
    database::import::parse_and_create_from_sql(db.pool(), project_id, &sql_content, &sql_dialect)
        .await
}
...
pub async fn db_save_table_preferences(
    ...
    db.save_table_preferences(project_id, preferences)
        .await
