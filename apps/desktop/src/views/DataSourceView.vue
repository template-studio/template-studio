<template>
  <div class="datasource-view">
    ...
    <a-modal
      ...
      :title="statusDatasource ? `${statusDatasource.name} - 连接状态` : '连接状态'"
      :footer="null"
      width="500px"
    >
      <a-spin :spinning="statusLoading">
        ...
        <a-empty v-else-if="!statusLoading" description="无法获取连接状态" />
      </a-spin>
    </a-modal>
...
// 打开连接状态弹窗
const openStatus = async (datasource) => {
  ...
  try {
    statusData.value = await datasourcesApi.getConnectionStatus(datasource)
  } catch (error) {
