...
export const Alova = createAlova({
  ...
  responded: {
    onSuccess: async (response, method) => {
      // 检查响应类型，避免将 HTML 解析为 JSON
      const contentType = response.headers?.get('content-type') || '';

      if (contentType.includes('text/html')) {
        console.error('API返回了HTML页面，可能是404或服务器错误');
        const error = new Error('Invalid response: received HTML instead of JSON');
        throw error;
      }
      ...
      // 统一信封：code:0 成功，负载在 data（过渡兼容已移除，见 dev-docs/api-envelope-analysis.md）
      ...
      if (code === 401) {
        ...
        // 可按需处理错误，不一定需要弹出 message
        Message?.error(message);
        throw new Error(message);
      }
    },
    onError: (error, method) => {
      console.error('Alova请求错误:', error);
      // @ts-ignore
      const Message = window.$message;
