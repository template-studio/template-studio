...
impl TemplateAnalysisService {
    ...
    /// 分析模板中的变量使用情况
    pub async fn analyze_variables(&self, template_id: i64) -> Result<VariableAnalysisResponse, AppError> {
        let template_root_path = self.base_path.join(template_id.to_string());
        let template_path = template_root_path.join("src");

        tracing::info!("分析模板变量，模板ID: {}, 路径: {:?}", template_id, template_path);
        ...
        self.collect_template_files(&analysis_path, &mut files).await;

        tracing::info!("找到 {} 个文件需要分析", files.len());

        // 检测变量
        let mut detected_variables: std::collections::HashMap<String, DetectedVariable> = std::collections::HashMap::new();
        ...
        for file_path in &files {
            ...
            // 使用正则表达式检测 Tera 模板变量
            // 匹配 {{ variable_name }} 或 {{ object.field }} 或 {{ variable | filter }}
            // 支持变量名：字母、数字、下划线，支持点号访问属性
            let re = match regex::Regex::new(r"\{\{\s*([a-zA-Z_][\w\.]*)(?:\s*\|\s*[^\}]*)?\s*\}\}") {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!("正则表达式编译失败: {:?}", e);
                    continue;
                }
            };
            ...
            for captures in re.captures_iter(&content) {
                if let Some(var_name) = captures.get(1) {
                    ...
                    let context = captures.get(0).map(|m| m.as_str().to_string()).unwrap_or_default();
                    ...
                    detected_variables
                        ...
                        .or_insert_with(|| DetectedVariable {
                            ...
                            suggestions: format!("建议使用 string 类型，或根据实际用途选择其他类型"),
                        });
        ...
        // 读取已定义的变量
        let defined_variables = self.load_defined_variables(template_id).await.unwrap_or_default();
        ...
        tracing::info!("分析完成: 检测到 {} 个变量，已定义 {} 个变量，缺失 {} 个变量",
            total_count, defined_variables.len(), missing_variables.len());
    ...
    async fn load_defined_variables(&self, template_id: i64) -> Result<Vec<String>, AppError> {
        let variables_file_path = self.base_path
                ...
                match serde_json::from_str::<serde_json::Value>(&content) {
                    Ok(value) => {
                        if let Some(obj) = value.as_object() {
                            ...
                            tracing::info!("加载到 {} 个已定义变量: {:?}", variable_names.len(), variable_names);
                            Ok(variable_names)
                        } else {
    ...
    async fn collect_template_files(&self, dir_path: &std::path::Path, files: &mut Vec<std::path::PathBuf>) {
                ...
                while let Ok(Some(entry)) = entries.next_entry().await {
                    ...
                    if path.is_file() {
                        ...
                        if let Some(ext) = path.extension() {
                            ...
                            if ["go", "rs", "py", "js", "ts", "java", "html", "htm", "tml", "tera", "txt", "md", "css", "json", "xml"].contains(&ext_str) {
