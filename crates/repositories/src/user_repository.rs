...
impl UserRepository {
    ...
    pub async fn create_user(&self, username: &str, password_hash: &str, email: &str) -> Result<i64> {
    ...
    pub async fn update_user(&self, id: i64, email: Option<&str>, avatar: Option<&str>, status: Option<i8>) -> Result<bool> {
        ...
        if let Some(v) = email { query = query.bind(v); }
