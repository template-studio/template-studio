...
impl TemplateRepository {
    ...
    pub async fn list(&self, query: &TemplateListQuery) -> Result<PagedResponse<Template>> {
        ...
        let (templates, total) = if let Some(language_id) = query.language_id {
            ...
            let count_sql = format!(
                "SELECT COUNT(*) FROM templates t WHERE t.category_id = ?{}",
                vis_filter
            );
            let total: i64 = sqlx::query_scalar(&count_sql)
                .bind(category_id as i64)
            ...
            let count_sql = format!(
                "SELECT COUNT(*) FROM templates t WHERE t.is_featured = ?{}",
                vis_filter
            );
            let total: i64 = sqlx::query_scalar(&count_sql)
                .bind(is_featured)
    ...
    pub async fn get_template_languages(
        &self,
        template_id: i64,
    ) -> Result<Vec<template_studio_shared::models::template::TemplateLanguageItem>> {
        let languages =
            sqlx::query_as::<_, template_studio_shared::models::template::TemplateLanguageItem>(
                r#"
    ...
    /// 获取指定分类下的模板（限制数量）
    pub async fn get_by_category_limit(
        &self,
        category_id: i64,
        limit: u32,
    ) -> Result<Vec<Template>> {
    ...
    /// LIKE 模式转义（配合 ESCAPE '|' 使用）：用户输入的 % _ | 不再干扰匹配语义
    ...
    /// 列出用户的模板（全部条件参数化绑定，不拼接用户输入）
    pub async fn list_user_templates(
        &self,
        owner_id: i64,
        query: &UserTemplateListQuery,
    ) -> Result<PagedResponse<Template>> {
        let page = query.page.unwrap_or(1).max(1);
        let page_size = query.page_size.unwrap_or(20).min(100);
        let offset = (page - 1) * page_size;

        let mut where_clauses = vec!["owner_id = ?".to_string()];
        if query.visibility.is_some() {
            where_clauses.push("visibility = ?".to_string());
        }
        if query.keyword.is_some() {
            where_clauses
                .push("(name LIKE ? ESCAPE '|' OR description LIKE ? ESCAPE '|')".to_string());
        }
        ...
        let mut q = sqlx::query_as::<_, Template>(&list_sql).bind(owner_id);
        if let Some(ref vis) = query.visibility {
            q = q.bind(vis.clone());
        }
        if let Some(ref kw) = query.keyword {
            let pat = Self::like_pattern(kw);
        ...
        let count_sql = format!("SELECT COUNT(*) FROM templates WHERE {}", where_sql);
        let mut count_q = sqlx::query_scalar::<_, i64>(&count_sql).bind(owner_id);
        if let Some(ref vis) = query.visibility {
            count_q = count_q.bind(vis.clone());
        }
        if let Some(ref kw) = query.keyword {
            let pat = Self::like_pattern(kw);
            count_q = count_q.bind(pat.clone()).bind(pat);
        }
    ...
    pub async fn list_pending_templates(
        ...
        let total: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM templates WHERE visibility = 'pending' AND status = 'active'",
        )
        .fetch_one(&self.pool)
        .await
        .unwrap_or(0);
    ...
    /// 获取公开模板列表（客户端，visibility=public 且 status=active；条件参数化绑定）
    pub async fn list_public_templates(
        ...
        let mut where_clauses = vec![
            ...
            "EXISTS (SELECT 1 FROM template_versions WHERE template_id = templates.id AND is_latest = true)".to_string(),
        ];
        if query.keyword.is_some() {
            where_clauses
                .push("(name LIKE ? ESCAPE '|' OR description LIKE ? ESCAPE '|')".to_string());
        ...
        let mut q = sqlx::query_as::<_, Template>(&list_sql);
        if let Some(ref kw) = query.keyword {
            let pat = Self::like_pattern(kw);
            q = q.bind(pat.clone()).bind(pat);
        }
    ...
    pub async fn review_template(
        ...
        let new_status = if action == "reject" {
            "rejected"
        } else {
            "active"
        };
    ...
    pub async fn increment_download_count(&self, template_id: i64) -> Result<()> {
        sqlx::query(
            "UPDATE templates SET download_count = COALESCE(download_count, 0) + 1 WHERE id = ?",
        )
        .bind(template_id)
        .execute(&self.pool)
        .await?;
