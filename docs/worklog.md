...
## 2026-09-10 转换工作台补渲染预览（任务 #140）

**变更内容：** 中栏文件预览 segmented 新增「渲染」视图(置于首位):模板化内容经 render_string_content 真引擎(与编辑器预览同源)按启用变量默认值注入渲染,展示生成效果;渲染失败显示错误横幅不阻断;变量默认值/文件/模式变化自动重渲,状态失效时清理;切换文件重置回模板化视图。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过;浏览器实测 segmented 三视图与渲染态切换正常(无后端环境优雅降级为错误横幅);真渲染需桌面端。

## 2026-09-10 转换工作台整体渲染预览（任务 #141）

**变更内容：** 中栏新增「渲染预览」页签:左侧全部输出文件列表(文件名+路径+替换数徽标,自动选中首个有替换的文件),右侧该文件经 render_string_content 真引擎按启用变量默认值渲染的完整结果(行号视图,3000 行截断);懒加载缓存(变量/清单变更自动失效重渲);进入页签无结果时自动生成替换结果;渲染失败显示错误横幅。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过;浏览器实测页签/文件列表/替换徽标/自动选中正常(渲染内容需桌面端后端)。

## 2026-09-10 渲染预览接入编辑器同款代码视图（任务 #142）

**变更内容：** ①新增公共只读代码查看器 CodeViewer(CodeMirror+Dracula+按扩展名语法高亮+行号+当前行高亮,配置与编辑器 TemplatePreview 同源,17 种语言映射);②转换工作台渲染预览页重做:内容区换 CodeViewer,头部对齐编辑器(路径+Cloud 引擎标签+渲染耗时+替换数+复制按钮),错误面板对齐编辑器(错误类型中文文案/行号定位/信息/上下文,红色面板);③文件预览「渲染」模式同步换 CodeViewer;移除原朴素 pre 行号视图。
...
## 2026-09-10 文件内容区重复路径头移除（任务 #151 补2）

**变更内容：** 文件内容区内部路径头与中栏面包屑重复,删除内部头,关闭按钮挪入面包屑行(seg 左侧)。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 重点文件视图重设计（任务 #152）

**变更内容：** 重点文件从侧栏(250px 挤压)移回主内容区双栏:左侧宽幅勾选树(目录级联),右侧策略面板(栏头+说明+下一步/重新分析 CTA+暴露策略单选+计数);focus 视图下隐藏 SideBar 让满宽;中途修复搬迁脚本误吞变量模板(重写干净结构并放回侧栏)。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过(变量/源码/模板/重点四视图结构完整)。

## 2026-09-10 规则独立视图:内置为底可覆盖（任务 #153）

**变更内容：** ①Rust 三命令:convert_rules_get(内置原文+覆盖原文+生效包,BUILTIN_SOURCES 常量回显)/convert_rules_save(解析校验 RulePack+id 一致,原子写用户规则目录)/convert_rules_reset(删覆盖恢复内置);RulePack/ConstantRule 补 Serialize;②前端 Activity Bar 新增「规则」(FilterFilled):主内容区视图(侧栏隐藏)——左包列表(五内置,当前使用「当前」标,已覆盖「覆盖」徽标),右 JSON 编辑(等宽 textarea)+保存覆盖/恢复内置/重新扫描(当前包时);覆盖即 all_packs 同 id 合并,重扫描生效。

**涉及文件：** `src-tauri/src/commands/convert.rs`、`src-tauri/src/lib.rs`、`src/views/convert/index.vue`

**验收结果：** cargo test 全绿;pnpm build 通过。

## 2026-09-10 规则视图样式修正（任务 #153 补）

**变更内容：** 规则列表项改横向行布局(名称+当前+徽标同行),修徽标折行成方块问题;列表卡片化/编辑器代码底色等精修。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 规则列表说明文案移除（任务 #153 补2）

**变更内容：** 底部长说明移除,改为「内置规则包」栏头悬停 tooltip。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 规则视图左右栏头对齐（任务 #153 补3）

**变更内容：** 左列表栏头与右工具栏统一 40px 高同顶边对齐。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 中栏头部按视图上下文显示（任务 #153 补4）

**变更内容：** seg/关闭/构建验证仅在源代码/模板视图显示,规则/重点/变量视图不再残留上一文件的控件。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 面包屑按视图显示（任务 #153 补5）

**变更内容：** 切到规则/重点/变量视图时面包屑显示视图名,不再残留上一选中文件路径。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 移除中栏头部关闭按钮（任务 #153 补6）

**变更内容：** 文件预览无独立关闭态(切换视图/点其它文件自然切换),移除头部 X 按钮及 closePreview。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 渲染视图收窄+文件树右键菜单（任务 #153 补7）

**变更内容：** ①「渲染」seg 选项仅在模板视图显示(源代码视图只有模板化/原文);②源码/模板树行右键菜单:预览/打开(模板化)/保留剔除(文件)/全部剔除保留(目录)/复制路径,自定义浮层+越界钳制。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 源码区移除视图切换（任务 #153 补8）

**变更内容：** 源码与模板文件非一一对应(大量剔除),源码区固定只读原文,模板化/渲染切换仅存于模板视图。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 模板区移除原文选项（任务 #153 补9）

**变更内容：** 模板区文件本身即模板,移除「原文」按钮,seg 仅 模板化|渲染;源码区固定原文无 seg。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 模板区改编辑器式分栏（任务 #153 补10）

**变更内容：** 废弃 模板化/渲染 切换交互——模板区改为左右分栏同时展示:左「模板」(占位符高亮+替换数),右「渲染」(真引擎默认值注入,CodeViewer);进入模板视图选中文件即自动渲染(变量默认值变化联动);源码区维持只读原文;「模板化」语义更名「模板」。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 分栏拖拽+渲染快捷+移除构建验证（任务 #153 补11）

**变更内容：** ①模板|渲染分栏支持拖拽调宽(20%-80%,localStorage 记忆,悬停手柄变品牌绿);②渲染栏头加「立即重新渲染」图标,两栏右键菜单(重新渲染/复制渲染结果/复制模板内容);③移除构建验证按钮/弹窗/逻辑——转换属临时工作区,构建验证归属编辑器项目区(#145)。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过,构建相关引用清零。

## 2026-09-10 分栏拖拽修复+显隐开关（任务 #153 补12）

**变更内容：** ①拖拽失效修复(首列被 flex:1 压制,补 flex:none);②渲染栏可折叠(编辑器预览式:渲染头收起箭头/模板头"‹渲染"展开,localStorage 记忆);③中栏头部侧栏显隐开关(编辑器文件树开关式,localStorage 记忆)。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 渲染栏收起后模板列撑满（任务 #153 补13）

**变更内容：** 渲染栏收起时模板列宽度改 100%(原固定百分比不扩张)。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 渲染栏收起窄条化（任务 #153 补14）

**变更内容：** 收起效果完全对齐编辑器预览面板:收起保留 40px 窄容器(仅居中展开按钮),图标换编辑器同款 ionicons5 ChevronBack/Forward;模板列收起态 calc(100%-45px) 撑满;移除临时左右箭头方案。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 右键菜单统一（任务 #153 补15）

**变更内容：** 模板树右键补齐与源码树同构操作(剔除移出模板/保留/复制模板内容/在源码区查看原文/目录批量/复制路径);模板与渲染分栏右键补「在源码区查看原文」,模板分栏补「剔除该文件」。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 内容区右键菜单对齐编辑器（任务 #153 补16）

**变更内容：** 参考编辑器 CodeMirror 内容区右键:模板分栏右键=复制/粘贴/全选+选中时「原样显示 {% raw %}」(选中文本包裹 raw 块,免变量替换)+立即渲染(Ctrl+R 语义)+剔除移出;源码区右键=复制/全选(只读);菜单升级编辑器样式(每项图标+divider 分组+13px/8px 12px)。

**涉及文件：** `src/views/convert/index.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 右键菜单失效修复（任务 #153 补17）

**变更内容：** 根因:App.vue 全局 contextmenu 拦截(document 捕获,白名单仅 .cm-editor 等)吞掉了工作台事件。白名单补 .cw-code/.cw-ctx/.ant-tree。

**涉及文件：** `src/App.vue`

**验收结果：** pnpm build 通过。

## 2026-09-10 右键图标导入修复（任务 #153 补18）

**变更内容：** 菜单图标组件(Eye/Stop/Check/FileSearch/MinusCircle/PlusCircle/Snippets/Select)导入丢失致运行时 ReferenceError,补齐 import。
